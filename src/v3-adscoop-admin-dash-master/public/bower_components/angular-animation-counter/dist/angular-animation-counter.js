...
.directive('incDecCounter', function($filter){

	//a function to convert the value to locale
	function generateLocale(value, locale, decimalValue){
		if(locale && Number.toLocaleString){
			return ($filter('number')(+value, decimalValue)).toLocaleString(locale);
		} else {
			return $filter('number')(value, decimalValue);
		}
	}
			...
			function setCounter(start, end){
				...
				var intFunc = setInterval(function(){
					...
					if((incr && counter >= end) || (!incr && counter <= end)){
						element.html(generateLocale(end, locale, $scope.decimal));
						clearInterval(intFunc);
						return;
					}
					element.html(generateLocale(Math.round(+counter), locale, $scope.decimal)); //showing the round off value in the interim
